...
It is the 20~\fbinv data set collected by the \ac{CMS} detector at $\sqrt{s}=8~\TeV$ that is used to search for evidence of \ac{SUSY} in Chapters~\ref{chap:sus13009} and~\ref{chap:sus13009results}.
...
The relatively long lived b-hadron can decay $\sim1$~cm away from the primary vertex, leading to a displaced vertex. 
...
\begin{figure}[htbp]
  ...
  \added{Here, and in several of the following plots, the units shown are not natural, indicated by the presence of $c$ in the specified unit.}
}
...
Although technically wrong, as energy is not a vector quantity, the two are used interchangeably as the absolute value of momentum and energy are; and we often take the magnitude of the missing transverse momentum vector, a scalar quantity.}
...
\deleted{Similarly, every physics analysis uses a trigger (or triggers) suited to the topology under investigation.}
