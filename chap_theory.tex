...
which shows many different experimental measurements and theory expectations \replaced{agreeing}{agree} over 6 orders of magnitude.
...
A similar table exists for the anti-particles of the leptons and quarks\replaced{. There are both positive and negative energy solutions of Equation~\ref{eqn:Dirac}, so, rather than interpreting the negative solutions as particles travelling backwards in time, }{, a consequence of Equation~\ref{eqn:Dirac}, which has both positive and negative energy solutions. 
...
This difference is due to the colour charge that quarks carry: they interact with the strong force\added{. The force carrier of the strong force, the gluon, is both massless and self-interacting, which results} in colour confinement and hadronization. \added{Rather than separate a quark from another quark or gluon into a `free' quark, it is more 
energetically favourable for a quark--anti-quark pair to pop out of the vacuum via the exchange of a gluon, creating a hadronization process.} 
Both quarks and \replaced{leptons}{gluons} exist in three families or generations, with each subsequent family increasing in mass.
...
The photon ($\gamma$) mediates the electromagnetic interaction and the heavy $\W^{\pm}$ and $\Z$ bosons mediate the weak interaction, through the mixing of the gauge fields when the respective forces are unified. There are 8 \replaced{coloured}{colourless} gluons ($g$) that mediate the strong force.
...
Noether's theorem~\cite{bib:Noether1918} states that \added{continuous} symmetries lead to conserved quantities in nature.
...
It is certainly reasonable that we should expect a theory to give the same answer 10 minutes ago as it does now; and in 10 minutes, or 10 years, or 10 millennia -- any Lagrangian which is not invariant under time transformations should perhaps be revised\footnote{\added{Unless the Lagrangian is not symmetric under \ac{CP} symmetry: for \ac{CPT} must always be invariant.}}.
...
The fermion wavefunctions are then written in terms of left-handed states (the electroweak doublet), and right-handed states (the electroweak singlet).
...
The convention is to choose the \ac{VeV} as 
\begin{equation}
...
\added{which} is chosen to be consistent with the ground state of the vacuum.
...
The three lost degrees of freedom correspond to the longitudinal polarizations of the new massive \added{$W^{\pm}$ and $Z$} bosons.
...
the same Higgs \replaced{field}{boson} can generate their masses.
...
The \ac{CP} violation in the kaon\added{, $B_{0}$} and $D_{0}$ meson sectors is not enough to explain why only matter remains from the Big Bang, 
in which matter and antimatter are thought to have been produced in even quantities. 
Physics \ac{BSM} is necessary to introduce another source of it in order to explain \replaced{why we}{we why} are here at all; why all matter did not annihilate soon after it was created.
...
%atmospheric
\added{Atmospheric neutrinos are produced in cosmic ray interactions in the atmosphere (resulting largely from charged pion decay), in the ratio of approximately 2:1 of $\nu_{\mu}$:$\nu_{e}$. 
There is similarly observed to be a deficit of the muon-like weak eigenstate in experiments on (or under) the ground~\cite{Fukuda:1998ub,PhysRevLett.81.1562}.
Experiments using neutrinos from accelerator and reactor sources have seen similar behaviours~\cite{An:2012eh,1674-1137-37-1-011001,Abe:2013hdq,Abe:2015awa}.}
...
Although possible, this is very unnatural, and drives \deleted{the} much of the theoretical motivation for \ac{BSM} physics.



\ac{SUSY}~\cite{susyMot1,susyMot2} is one example of an extension to the \ac{SM} which can solve many of its problems, and is the subject of \added{the} remainder of this chapter, as well as the analysis work done in this thesis. 
...
The generated particles, or sparticles, have \deleted{the} all of the same quantum numbers as those particles they have been generated with (but for the spin) -- so they have the same mass. 
...
\deleted{In the \ac{MSSM}, e}Every \ac{SM} particle gets a supersymmetric particle (sparticle) partner which has the same quantum numbers but differs a half integer unit in spin and has a greater mass.\added{ In the \ac{MSSM}~\cite{Haber:1993wf}, there is the minimum number of new particle states, with one extra Higgs doublet.} 
...
There will instead be an imbalance in momentum, leading to a missing transverse \replaced{momentum}{energy} signature.

\added{Proton decay requires both lepton and baryon number 
violation, so 
certain models that have partial R-parity violation in either B or L can be compatible with limits on proton decay. 
These do not give rise to the same missing transverse momentum signatures as R-parity conserving \ac{SUSY}, as the \ac{LSP} becomes unstable eventually decays into \ac{SM} particles. 
There are dedicated searches for such models, and these are discussed elsewhere~\cite{ATLAS:2012kr,CMS-PAS-SUS-13-003,CMS:2013qda}.}
...
A more model independent approach has been adopted more recently \added{at \ac{CMS}}, with the use of \ac{SMS}~\cite{SMS1,SMS2,SMS3, SMSPaper}, which allow searches to be interpreted in the mass planes of various sparticles: in the mass of the \sTop{}, \sbottom, \squark and \chiOneZero for example.
...
Searches have ruled out huge swathes of phase space, see Fig.~\ref{fig:cmssmlimits} and Fig.~\ref{fig:susy2013stop} that show the limits on the \ac{CMSSM} and the combined \ac{CMS} results on the \sTop{}, \chiOneZero{} mass plane\added{, as shown} at SUSY 2013, respectively.
...
\footnote{\added{Recent work may indicate that a light stop is not necessarily the only route to `natural' SUSY; light higgsinos can also give a small amount of fine-tuning~\cite{Casas:2014eca}.}}.
...
Similarly, the \chiOneZero at the end of the decay chain will also be very soft, so \added{the} \MET in each event is also small. 
...
$\sTop \rightarrow b \chiOneZero f \bar{f}'$ can occur, where $f$ is any fermion, \added{and the intermediate top quark is produced off-shell.}
...
Figure~\ref{fig:stopDecayMassPlane} summarizes the decays of the top squark, showing the relevant decay for the different kinematic regimes of \sTop{} and \chiOneZero mass \added{in the \sTop{}--\chiOneZero{} mass plane}.
...
\begin{figure}[t!]
  ...
  \caption{Feynman diagrams to show two contributing processes to the decays of the top squark: $\sTop \rightarrow c \chiOneZero$ (left) and $\sTop \rightarrow b \chiOneZero f \bar{f}'$ (right). \added{The intermediate top quark on the right, $t^{*}$, is produced off-shell.}}
...
Chapters~\ref{chap:sus13009} and~\ref{chap:sus13009results} will describe a search for light stops decaying to a charm quark and a neutralino, using events with a monojet topology.
\added{This search is applicable to many new physics scenarios, being more usually applied to dark matter searches and the like. Here, it is interpreted for these particular third generation compressed \ac{SUSY} scenarios, and equally could be applied to other compressed \ac{SUSY} scenarios.
}
